from sql_alquemy import banco

class HotelModel(banco.Model):
    __tablename__ = 'hoteis'

    hotel_id = banco.Column(banco.String, primary_key=True)
    nome = banco.Column(banco.String(80))
    ranking = banco.Column(banco.Float(precision=1))
    diaria = banco.Column(banco.Float(precision=2))
    estado = banco.Column(banco.String(40))
    site_id = banco.Column(banco.Integer, banco.ForeignKey('sites.site_id'))

    def __init__(self, hotel_id, nome, ranking, diaria, estado, site_id):
        self.hotel_id = hotel_id
        self.nome = nome
        self.ranking = ranking
        self.diaria = diaria
        self.estado = estado
        self.site_id = site_id

    def json(self):
        return {
            'hotel_id': self.hotel_id,
            'nome': self.nome,
            'ranking': self.ranking,
            'diaria': self.diaria,
            'estado': self.estado,
            'site_id': self.site_id
        }

    @classmethod
    def find_hotel(cls, hotel_id):
        hotel = cls.query.filter_by(hotel_id=hotel_id).first() #SELECT * FROM hoteis WHERE hotel_id=hotel_id
        if hotel:
            return hotel
        return None

    def save_hotel(self):
        banco.session.add(self)
        banco.session.commit()

    def update_hotel(self, nome, ranking, diaria, estado, site_id):
        self.nome = nome
        self.ranking = ranking
        self.diaria = diaria
        self.estado = estado
        self.site_id = site_id

    def delete_hotel(self):
        banco.session.delete(self)
        banco.session.commit()